...
export function createMap(mapID: MapID, options: Viewer.ConstructorOptions = {}) {
...
export function addPointImage(viewer: Viewer, url: string, position: Cesium.Cartesian3, width: number, height: number) {
...
export function addWallImage({viewer, id, group, image, points}: option) {
  ...
  let positions = Cartesian3.fromDegreesArrayHeights([
  ...
  let entity = new Entity({
    ...
    wall: {
      ...
      minimumHeights: [10000.0, 10000.0],
    }
  });
